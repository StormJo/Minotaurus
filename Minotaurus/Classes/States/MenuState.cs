...
namespace Minotaurus.Classes.States
{
    internal class MenuState : IState
    {
        ...
        public void Draw(SpriteBatch spriteBatch)
        {
            ...
            spriteBatch.DrawString(MinoMaze.Arial, "Press any key to start", new Vector2(285, 240), Color.Gray, 0, Vector2.Zero, 1f, SpriteEffects.None, 0);
        ...
        public void Update(GameTime gameTime)
        {
            ...
            {
                MinoMaze.ChangeState(new GameState(3));
            }
            else if (Keyboard.GetState().GetPressedKeys().Length > 0)
            {
                Random random = new Random();
                int randomState = random.Next(1, 4);
                MinoMaze.ChangeState(new GameState(randomState));
            }
